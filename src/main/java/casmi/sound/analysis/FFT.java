...
public class FFT extends FourierTransform
{
  ...
  public void scaleBand(int i, float s)
  {
    if (s < 0)
    {
      Sound.error("Can't scale a frequency band by a negative value.");
      return;
    }
    if (spectrum[i] != 0)
    {
  ...
  public void setBand(int i, float a)
  {
    if (a < 0)
    {
      Sound.error("Can't set a frequency band to a negative value.");
      return;
    }
  ...
  public void forward(float[] buffer)
  {
    ...
    {
      Sound.error("FFT.forward: The length of the passed sample buffer must be equal to timeSize().");
      return;
    }
    doWindow(buffer);
  ...
  public void forward(float[] buffReal, float[] buffImag)
  {
    ...
    {
      Sound.error("FFT.forward: The length of the passed buffers must be equal to timeSize().");
      return;
    }
  ...
  public void inverse(float[] buffer)
  {
    ...
    {
      Sound.error("FFT.inverse: the passed array's length must equal FFT.timeSize().");
      return;
    }
    // conjugate
